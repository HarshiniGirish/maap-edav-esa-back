...
class BiosarGeoLoader(CMRLoader):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.biosar_spec={
                "BIOSAR1":{
                    "bucket":"biosar1",
                    "regex":"((\_\w{3}\_\w{1}\_geo)|(\_amplitude\_geo))"
                },
                "BIOSAR2":{
                    "bucket":"biosar2",
                    "regex":"\_\w{1}\_geo"
    ...
    def getList(self):
        ...
        for obj in bucket.objects.filter( Prefix = "edav_prepocessed_data/%s"%(self.biosar_spec[self.args["dataset"]]["bucket"]) ):
    ...
    def load(self, filename,datasetlist):
        ...
        try:
            if "kz" in filename:
                ...
                granule_ur=re.split(r"%s"%(self.biosar_spec[self.args["dataset"]]["regex"]),os.path.basename(filename))[0]+".tiff"
            params={
            ...
            if r.status_code==200 and r.json()["hits"]==1:
                ...
                data_json = {}
                send_json = {}
                if self.args["dataset"] == "BIOSAR1":
                    if "amplitude" in filename:
                        concept_id=json_feature [ "meta" ][ "concept-id" ]+"geo"+os.path.basename(filename).split("_")[4]
                    else:
                        concept_id=json_feature [ "meta" ][ "concept-id" ]+"geo"+os.path.basename(filename).split("_")[5]
                elif self.args["dataset"] == "BIOSAR2":
                    if "kz" in filename:
                        concept_id=json_feature [ "meta" ][ "concept-id" ]+"geo"
                    else:
                        concept_id=json_feature [ "meta" ][ "concept-id" ]+"geo"+os.path.basename(filename).split("_")[4]
                elif self.args["dataset"] == "BIOSAR3":
                    if "kz" in filename:
                        concept_id=json_feature [ "meta" ][ "concept-id" ]+"geo"
                    else:
                        concept_id=json_feature [ "meta" ][ "concept-id" ]+"geo"+os.path.basename(filename).split("_")[4]
                ...
                polarization=os.path.basename(filename).split("_")[3]
                if self.args["dataset"] == "BIOSAR1":
                    if "amplitude" in filename:
                        data_json[ concept_id ].update( { "datasetId" : datasetlist [ 0 ][ "short_name" ]+"_geo" } )
                        data_json[ concept_id ].update( { "subDatasetId" : datasetlist [ 0 ][ "short_name" ]+"_geo_"+product+"_"+polarization+"_"+os.path.basename(filename).split("_")[4] } )
                    else:
                        data_json[ concept_id ].update( { "datasetId" : datasetlist [ 0 ][ "short_name" ]+"_geo" } )
                ...
                scenetype,scene_value,subregion, subregion_value ,wcs_val= self.getSceneparams(self.extractAttributes(json_feature["umm"]["AdditionalAttributes"],"Scene Name"),datasetlist [ 0 ][ "title" ],["N/A"])
